...
class CRMApp:
    """Classe pour l'interface utilisateur du CRM avec Tkinter."""
    ...
    def __init__(self, root):
        ...
        self.label_nom = tk.Label(root, text="Nom")
        ...
        self.label_telephone = tk.Label(root, text="Téléphone")
        self.label_telephone.grid(row=2, column=0)
        self.entry_telephone = tk.Entry(root)
        self.entry_telephone.grid(row=2, column=1)

        self.label_categorie = tk.Label(root, text="Catégorie (SEO/Web/Autre)")
        self.label_categorie.grid(row=3, column=0)
        self.entry_categorie = tk.Entry(root)
        self.entry_categorie.grid(row=3, column=1)

        self.label_employes = tk.Label(root, text="Nombre d'employés")
        self.label_employes.grid(row=4, column=0)
        self.entry_employes = tk.Entry(root)
        self.entry_employes.grid(row=4, column=1)

        # Bouton pour ajouter un client
        self.btn_ajouter = tk.Button(root, text="Ajouter Client", command=self.ajouter_client)
        self.btn_ajouter.grid(row=5, column=0, columnspan=2)

        # Configuration du Treeview pour afficher les clients
        self.tree = ttk.Treeview(root, columns=("nom", "email", "telephone", "categorie", "employes", "cout_ht", "cout_ttc"), show="headings")
        ...
        self.label_total_clients = tk.Label(root, text="Total Clients : 0", font=("Helvetica", 12))
        self.label_total_clients.grid(row=7, column=0, sticky="w")

        self.label_chiffre_affaire = tk.Label(root, text="Chiffre d'Affaires : 0.00 €", font=("Helvetica", 16, "bold"))
        self.label_chiffre_affaire.grid(row=7, column=1, sticky="e")

        # Bouton pour actualiser les statistiques
        self.btn_actualiser_stat = tk.Button(root, text="Actualiser les Statistiques", command=self.afficher_statistiques)
        self.btn_actualiser_stat.grid(row=8, column=0, columnspan=2)

        # Bouton pour actualiser l'affichage des clients
        self.btn_actualiser_clients = tk.Button(root, text="Actualiser les Clients", command=self.afficher_clients)
        self.btn_actualiser_clients.grid(row=9, column=0, columnspan=2)
